...
def prune_path(path):
    ...
    while i < len(pruned_path) - 2:
        p1 = p_holder(pruned_path[i])
        p2 = p_holder(pruned_path[i + 1])
        p3 = p_holder(pruned_path[i + 2])
...
# place holder for three consecutive waypoints
def p_holder(p):
    return np.array([p[0], p[1], 1.]).reshape(1, -1)


# Collinearity test for removing extra waypoints
...
# local position
def position(local_position, offsets):
